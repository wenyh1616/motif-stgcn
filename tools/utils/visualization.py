...
# ft2.loadFontData('/usr/share/fonts/opentype/noto/NotoSansCJK-Black.ttc', 0)
...
def stgcn_visualize(pose,
    ...
    for t in range(T):
        ...
        # myfont = FontProperties(fname='/usr/share/fonts/opentype/noto/NotoSansCJK-Black.ttc')
        # ax.set_ylabel(u"错误动作分类数目", fontproperties=myfont)
        cv2.putText(frame, '视频', *params)
        cv2.putText(skeleton, '姿态估计（输入骨架）', *params)
        cv2.putText(skeleton_result, '注意力+实时估计', *params)
        cv2.putText(rgb_result, '输出+视频', *params)

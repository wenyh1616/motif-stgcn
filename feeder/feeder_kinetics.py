...
class Feeder_kinetics(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        _,pathnow = os.path.split(self.data_path)
        if self.debug:
        ...
        # choose sub-kinetics
        use = ['belly dancing', 'punching bag', 'capoeira', 'squat', 'windsurfing', 'skipping rope', 'swimming backstroke', 'hammer throw', 'throwing discus', 'tobogganing', 'hopscotch',
        ...
        outpath = '../../datasets/kinetics-skeleton-sub/'
        dict = {}
        for labelidx in label_info:
            hasskel = label_info[labelidx]['has_skeleton']
            if hasskel:
                msg = label_info[labelidx]['label']
                if msg in use:
                    label_index = use.index(msg)
                    datadict = {'has_skeleton': label_info[labelidx]['has_skeleton'], 'label': msg, 'label_index': label_index}
                    dict[(labelidx)] = datadict
                    srcfile = os.path.join(self.data_path,labelidx + '.json')
                    dstfile = os.path.join(outpath, pathnow, labelidx + '.json')
                    if os.path.isfile(srcfile):
                        with open(srcfile) as f:
                            src_info = json.load(f)
                            f.close()
                            if not os.path.isfile(dstfile):
                                dst_info = {'data': src_info['data'], 'label': src_info['label'], 'label_index': label_index}
                                json.dump(dst_info, open(dstfile, "w"))

        if 'train' in label_path:
            json.dump(dict, open(outpath + pathnow + '_label.json', "w"))
    ...
    def __getitem__(self, index):
        ...
        return data_numpy, label, labelstr
